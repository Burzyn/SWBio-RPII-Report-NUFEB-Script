...
# SUBSTRATE SAMPLE MAKER ============================================================================================

random_numbers_list = []
new_Gexp_folder =  f"{new_folders_directory}/Substrate"
os.makedirs(new_Gexp_folder, exist_ok=True)
for rep in replciate_list:
    new_Rep_folder = f"{new_Gexp_folder}/Sr_R{rep}"
    os.makedirs(new_Rep_folder, exist_ok=True)
    for sample in sample_list:
        new_sam_folder = f"{new_Rep_folder}/Sr_R{rep}_S{sample}"
        ...
        if sample == 1:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 0.5e-5")
        if sample == 2:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 1.5e-5")
        if sample == 3:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 2.5e-5")
        if sample == 4:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 3.5e-5")
        if sample == 5:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 4.5e-5")
        if sample == 6:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 5.5e-5")
        if sample == 7:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 6.5e-5")
        if sample == 8:
            updated_NUFEB = update2_NUFEB.replace("fix growth_het HET nufeb/growth/het sub 3.5e-5", "fix growth_het HET nufeb/growth/het sub 7.5e-5")
...
savedir_seed_file = f"{home_exp_directory}/Experiment_SubstrateSeeds_xlsx.xlsx"
...
cd /home/piotr/Desktop/NUFEB/Data/NUFEB_Experiment_5
