def the_cellmaxsurface_finder(input_path):
    ...
    while True:
        if iterator > 0:
            for index, row in table_sorted_Z.iterrows():
                if row["All_Cells"] >= cutoff_cell_max:
                    last_timepoint = row["Timepoint"]
                    iterator = 0
                    break
    ...
    savedir2 = f"{dirref}CellMaxSurfaceOutput_{folder_name}"
